...
export default function DashboardLayout() {
  ...
  const navBtn = ({ isActive }: { isActive: boolean }) =>
    `rounded-full px-4 py-2 text-[10px] font-black uppercase tracking-[0.16em] transition ${isActive
    ...
    <main className={`${bg} min-h-screen overflow-x-hidden font-sans antialiased selection:bg-violet-400/30`}>
      ...
      <header className="fixed inset-x-0 top-0 z-40 px-1 py-1 lg:px-8">
        <div className={`mx-auto flex max-w-7xl items-center gap-3 rounded-full border px-4 py-2.5 backdrop-blur-2xl lg:px-5 ${headerBg}`}>
          ...
          <button
            onClick={() => nav("/login")}
            className={`flex items-center gap-1.5 rounded-full border px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] transition ${isDark
          ...
          <div className="flex items-center gap-2">
            {/* Role badge */}
            <span
              className={`hidden sm:flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest ${canAdmin ? "border-violet-400/30 text-violet-300" : "border-white/10 text-zinc-500"
                }`}
            >
      ...
      {/* FOOTER */}
      <footer
        className={`relative overflow-hidden border-t ${isDark ? "border-white/10" : "border-violet-950/10"
          }`}
      >
